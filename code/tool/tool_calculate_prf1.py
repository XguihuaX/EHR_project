...
for entity in labels:
    input_file_path = f'/ihome/hdaqing/jul230/program/result/tools/{tool}_extracted_results_{entity}.csv.csv'
    ...
    for i in range(len(true)):
        if pd.isna(true[i]):
            true_labels.append(set())
        else:
            true_labels.append(set(str(true[i]).split(', ')))

        if pd.isna(pred[i]):
            pred_labels.append(set())
        else:
            pred_labels.append(set(str(pred[i]).split(', ')))
    ...
    assert len(true_labels_flat) == len(
        pred_labels_flat), "Lengths of true_labels_flat and pred_labels_flat do not match!"
