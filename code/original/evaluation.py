...
config = LlamaConfig.from_pretrained("meta-llama/Llama-2-13b-chat-hf", cache_dir='/ihome/hdaqing/abg96/cache', torch_dtype=torch.float16, token=access_token)
...
model = transformers.LlamaForCausalLM.from_pretrained("meta-llama/Llama-2-13b-chat-hf", torch_dtype=torch.float16, cache_dir='/ihome/hdaqing/abg96/cache', device_map=device_map, offload_folder="offload", offload_state_dict=True, token=access_token)

device_map = infer_auto_device_map(model)

tokenizer = LlamaTokenizer.from_pretrained("meta-llama/Llama-2-13b-chat-hf", unk_token="<unk>", bos_token="<s>", eos_token="</s>", cache_dir='/ihome/hdaqing/abg96/cache', token=access_token)
...
input_file_path = f'/ihome/hdaqing/jul230/program/result/tools/rule_annotations_extracted_{entity}.csv'
output_file_path = f'/ihome/hdaqing/jul230/program/result/tools/evaluated_annotations_extracted_{entity}.csv'


df = pd.read_csv(input_file_path)
if 'evaluation reasoning' not in df.columns:
    df['evaluation reasoning'] = ''
